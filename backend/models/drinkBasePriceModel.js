// backend/models/drinkBasePriceModel.js
const db = require("../DB/db");

// เพิ่มราคาฐานเครื่องดื่ม
exports.create = async (drink_id, temperature, base_price) => {
  const sql = `
    INSERT INTO drink_base_prices (drink_id, temperature, base_price)
    VALUES (?, ?, ?)
  `;
...
exports.update = async (drink_id, temperature, base_price) => {
  const sql = `
    UPDATE drink_base_prices
    SET base_price = ?
    WHERE drink_id = ? AND temperature = ?
  `;
...
exports.getAll = async () => {
  const sql = `
    SELECT drink_id, temperature, base_price
    FROM drink_base_prices
    ORDER BY drink_id, temperature
  `;
